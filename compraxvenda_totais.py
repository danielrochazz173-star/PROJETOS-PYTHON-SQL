...
def buscar_totais(conn, mes, ano, supervisores=None, incluir_pedidos=False):
...
def gerar_html_totais(total_venda, total_compra, mes, ano, incluir_pedidos=False):
    """Gera HTML com totais no estilo da imagem"""
    mes_formatado = get_mes_ano_formatado(mes, ano)
    
    # Ajustar título baseado se inclui pedidos ou não
    if incluir_pedidos:
        titulo_mes = f"Totais - FAT + PEDIDOS - {mes_formatado}"
    else:
        titulo_mes = f"Totais - {mes_formatado}"
    ...
    # Calcular diferença (venda - compra)
    diferenca = total_venda - total_compra
    valor_diferenca = formatar_valor_completo(diferenca)
    
    # Calcular porcentagem da diferença (venda em relação à compra)
    if total_compra > 0:
        porcentagem = ((total_venda / total_compra) - 1) * 100
        porcentagem_formatada = f"{porcentagem:+.2f}%".replace('.', ',')
    else:
        porcentagem = 0
        porcentagem_formatada = "0,00%"
    
    # Determinar cor da diferença (verde se positivo, vermelho se negativo)
    cor_diferenca = "#4CAF50" if diferenca >= 0 else "#F44336"
    ...
    <style>
        * {{
        ...
        .card-diferenca::before {{
            ...
            width: 200%;
            height: 200%;
            background: radial-gradient(circle, {cor_diferenca}33 0%, transparent 70%);
            animation: pulse 4s ease-in-out infinite;
        }}
        
        .card-diferenca::after {{
            content: '';
            position: absolute;
            top: 0;
            left: 0;
            ...
            border-radius: 20px;
            padding: 2px;
            background: linear-gradient(135deg, {cor_diferenca}4D, {cor_diferenca}1A);
            -webkit-mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0);
            -webkit-mask-composite: xor;
            mask-composite: exclude;
            pointer-events: none;
        ...
        @media print {{
            ...
            .card::before, .card-diferenca::before {{
                animation: none;
            }}
            .footer {{
                margin-top: 40px;
...
<body>
    ...
    <div class="container-diferenca">
        <div class="card-diferenca">
            <div class="card-content">
                <div class="label">Diferença</div>
                <div class="valor-diferenca">{valor_diferenca}</div>
                <div class="valor-porcentagem">{porcentagem_formatada}</div>
            </div>
        </div>
    </div>
...
class InterfaceAnalise:
    ...
    def criar_aba_totais(self):
        ...
        checkbox_pedidos.grid(row=4, column=0, columnspan=2, pady=15, sticky=tk.W)
        
        # Botão gerar
        btn_gerar_totais = ttk.Button(main_frame, text="Gerar Totais", command=self.gerar_totais)
        btn_gerar_totais.grid(row=5, column=0, columnspan=2, pady=20)
        
        # Status
        self.status_label_totais = ttk.Label(main_frame, text="Pronto para gerar totais")
        self.status_label_totais.grid(row=6, column=0, columnspan=2, pady=10)
    ...
    def gerar_totais(self):
        ...
        try:
            ...
            try:
                ...
                # Buscar totais
                total_compra, total_venda = buscar_totais(conn, mes, ano, supervisores, incluir_pedidos)
                
                self.status_label_totais.config(text="Gerando relatório...")
                self.master.update()
                
                # Gerar HTML
                html_path = gerar_html_totais(total_venda, total_compra, mes, ano, incluir_pedidos)

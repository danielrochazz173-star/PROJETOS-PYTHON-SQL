...
def gerar_excel_simplificado(df_resumo, ano, caminho_arquivo):
    """
    Gera Excel com PROCV - aba Dados + abas com fórmulas
    
    REGRAS PARA FÓRMULAS EXCEL PT-BR:
    - Funções em português: SE, PROCV, CONT.SE, FALSO, VERDADEIRO
    - Ponto e vírgula (;) como separador de argumentos
    - FALSO/VERDADEIRO sem aspas
    - Sem @ antes de funções
    - Referências normais A1, B2 (sem referências estruturadas)
    - Fixar células importantes com $ ($F$1)
    ...
    # =========================================================================
    # ABA DADOS (fonte de dados para PROCV)
    # =========================================================================
    ws_dados = wb.create_sheet("Dados", 0)
    
    # Cabeçalhos da aba Dados
    headers_dados = [
        'RCA', 'Nome Vendedor', 
        'Faturamento Anual', 'Faturamento Média Ano', 'Faturamento Média Trimestre', 
        'Faturamento Mês Passado', 'Faturamento Mês Atual',
        'Margem Anual %', 'Margem Média Ano', 'Margem Média Trimestre',
        'Margem Mês Passado', 'Margem Mês Atual',
        'Mix Anual', 'Mix Média Ano', 'Mix Média Trimestre',
        'Mix Mês Passado', 'Mix Mês Atual'
    ]
    
    for col, header in enumerate(headers_dados, 1):
        cell = ws_dados.cell(row=1, column=col)
        cell.value = header
    ...
    for idx, row in df_resumo.iterrows():
        r = idx + 2
        ws_dados.cell(r, 1, row['rca']).border = border
        ws_dados.cell(r, 2, row['nome_vendedor']).border = border
        ws_dados.cell(r, 3, row['faturamento_anual']).number_format = '#,##0.00'
        ws_dados.cell(r, 3).border = border
        ws_dados.cell(r, 4, row['faturamento_medio_ano']).number_format = '#,##0.00'
        ws_dados.cell(r, 4).border = border
        ws_dados.cell(r, 5, row['faturamento_medio_trimestre']).number_format = '#,##0.00'
        ws_dados.cell(r, 5).border = border
        ws_dados.cell(r, 6, row['faturamento_mes_passado']).number_format = '#,##0.00'
        ws_dados.cell(r, 6).border = border
        ws_dados.cell(r, 7, row['faturamento_mes_atual']).number_format = '#,##0.00'
        ws_dados.cell(r, 7).border = border
        ws_dados.cell(r, 8, row['margem_anual_pct'] / 100).number_format = '0.00%'
        ws_dados.cell(r, 8).border = border
        ws_dados.cell(r, 9, row['margem_media_ano'] / 100).number_format = '0.00%'
        ws_dados.cell(r, 9).border = border
        ws_dados.cell(r, 10, row['margem_media_trimestre'] / 100).number_format = '0.00%'
        ws_dados.cell(r, 10).border = border
        ws_dados.cell(r, 11, row['margem_mes_passado'] / 100).number_format = '0.00%'
        ws_dados.cell(r, 11).border = border
        ws_dados.cell(r, 12, row['margem_mes_atual'] / 100).number_format = '0.00%'
        ws_dados.cell(r, 12).border = border
        ws_dados.cell(r, 13, int(row['mix_anual'])).number_format = '#,##0'
        ws_dados.cell(r, 13).border = border
        ws_dados.cell(r, 14, int(row['mix_medio_ano'])).number_format = '#,##0'
        ws_dados.cell(r, 14).border = border
        ws_dados.cell(r, 15, int(row['mix_medio_trimestre'])).number_format = '#,##0'
        ws_dados.cell(r, 15).border = border
        ws_dados.cell(r, 16, int(row['mix_mes_passado'])).number_format = '#,##0'
        ws_dados.cell(r, 16).border = border
        ws_dados.cell(r, 17, int(row['mix_mes_atual'])).number_format = '#,##0'
        ws_dados.cell(r, 17).border = border
    
    for col in range(1, 18):
        ws_dados.column_dimensions[get_column_letter(col)].width = 18
    ws_dados.freeze_panes = 'A2'
    
    # =========================================================================
    # ABA FATURAMENTO (com PROCV)
    # =========================================================================
    ws_faturamento = wb.create_sheet("Faturamento")
    ws_faturamento.merge_cells('A1:G1')
    ws_faturamento['A1'] = f"FATURAMENTO - {ano}"
    ws_faturamento['A1'].font = title_font
    ws_faturamento['A1'].alignment = center_align
    
    headers = ['RCA', 'Nome Vendedor', 'Anual', 'Média Ano', 'Média Trimestre', 'Mês Passado', 'Mês Atual']
    for col, header in enumerate(headers, 1):
        cell = ws_faturamento.cell(row=3, column=col)
        cell.value = header
        cell.fill = header_fill
        cell.font = header_font
        cell.alignment = center_align
    ...
    for idx in range(len(df_resumo)):
        r = idx + 4
        linha_dados = idx + 2  # Linha na aba Dados (linha 1 = cabeçalho, linha 2+ = dados)
        # RCA e Nome - copiar da aba Dados
        ws_faturamento.cell(r, 1).value = f'=Dados!A{linha_dados}'
        ws_faturamento.cell(r, 1).border = border
        ws_faturamento.cell(r, 2).value = f'=Dados!B{linha_dados}'
        ws_faturamento.cell(r, 2).border = border
        
        # Faturamento - referenciar diretamente da aba Dados (mais simples e confiável)
        ws_faturamento.cell(r, 3).value = f'=Dados!C{linha_dados}'
        ws_faturamento.cell(r, 3).number_format = '#,##0.00'
        ws_faturamento.cell(r, 3).alignment = right_align
        ws_faturamento.cell(r, 3).border = border
        
        ws_faturamento.cell(r, 4).value = f'=Dados!D{linha_dados}'
        ws_faturamento.cell(r, 4).number_format = '#,##0.00'
        ws_faturamento.cell(r, 4).alignment = right_align
        ws_faturamento.cell(r, 4).border = border
        
        ws_faturamento.cell(r, 5).value = f'=Dados!E{linha_dados}'
        ws_faturamento.cell(r, 5).number_format = '#,##0.00'
        ws_faturamento.cell(r, 5).alignment = right_align
        ws_faturamento.cell(r, 5).border = border
        
        ws_faturamento.cell(r, 6).value = f'=Dados!F{linha_dados}'
        ws_faturamento.cell(r, 6).number_format = '#,##0.00'
        ws_faturamento.cell(r, 6).alignment = right_align
        ws_faturamento.cell(r, 6).border = border
        
        ws_faturamento.cell(r, 7).value = f'=Dados!G{linha_dados}'
        ws_faturamento.cell(r, 7).number_format = '#,##0.00'
        ws_faturamento.cell(r, 7).alignment = right_align
        ws_faturamento.cell(r, 7).border = border
    
    ws_faturamento.column_dimensions['A'].width = 10
    ws_faturamento.column_dimensions['B'].width = 35
    for col in range(3, 8):
        ws_faturamento.column_dimensions[get_column_letter(col)].width = 18
    ws_faturamento.freeze_panes = 'A4'
    
    # =========================================================================
    # ABA MARGEM (com PROCV)
    # =========================================================================
    ws_margem = wb.create_sheet("Margem")
    ws_margem.merge_cells('A1:G1')
    ws_margem['A1'] = f"MARGEM % - {ano}"
    ...
    for idx in range(len(df_resumo)):
        r = idx + 4
        linha_dados = idx + 2
        ws_margem.cell(r, 1).value = f'=Dados!A{linha_dados}'
        ws_margem.cell(r, 1).border = border
        ws_margem.cell(r, 2).value = f'=Dados!B{linha_dados}'
        ws_margem.cell(r, 2).border = border
        
        # Margem - referenciar diretamente da aba Dados
        ws_margem.cell(r, 3).value = f'=Dados!H{linha_dados}'
        ws_margem.cell(r, 3).number_format = '0.00%'
        ws_margem.cell(r, 3).alignment = right_align
        ws_margem.cell(r, 3).border = border
        
        ws_margem.cell(r, 4).value = f'=Dados!I{linha_dados}'
        ws_margem.cell(r, 4).number_format = '0.00%'
        ws_margem.cell(r, 4).alignment = right_align
        ws_margem.cell(r, 4).border = border
        
        ws_margem.cell(r, 5).value = f'=Dados!J{linha_dados}'
        ws_margem.cell(r, 5).number_format = '0.00%'
        ws_margem.cell(r, 5).alignment = right_align
        ws_margem.cell(r, 5).border = border
        
        ws_margem.cell(r, 6).value = f'=Dados!K{linha_dados}'
        ws_margem.cell(r, 6).number_format = '0.00%'
        ws_margem.cell(r, 6).alignment = right_align
        ws_margem.cell(r, 6).border = border
        
        ws_margem.cell(r, 7).value = f'=Dados!L{linha_dados}'
        ws_margem.cell(r, 7).number_format = '0.00%'
        ws_margem.cell(r, 7).alignment = right_align
        ws_margem.cell(r, 7).border = border
    
    ws_margem.column_dimensions['A'].width = 10
    ws_margem.column_dimensions['B'].width = 35
    for col in range(3, 8):
        ws_margem.column_dimensions[get_column_letter(col)].width = 18
    ...
    for idx in range(len(df_resumo)):
        r = idx + 4
        linha_dados = idx + 2
        ws_mix.cell(r, 1).value = f'=Dados!A{linha_dados}'
        ws_mix.cell(r, 1).border = border
        ws_mix.cell(r, 2).value = f'=Dados!B{linha_dados}'
        ws_mix.cell(r, 2).border = border
        
        # Mix - referenciar diretamente da aba Dados
        ws_mix.cell(r, 3).value = f'=Dados!M{linha_dados}'
        ws_mix.cell(r, 3).number_format = '#,##0'
        ws_mix.cell(r, 3).alignment = right_align
        ws_mix.cell(r, 3).border = border
        
        ws_mix.cell(r, 4).value = f'=Dados!N{linha_dados}'
        ws_mix.cell(r, 4).number_format = '#,##0'
        ws_mix.cell(r, 4).alignment = right_align
        ws_mix.cell(r, 4).border = border
        
        ws_mix.cell(r, 5).value = f'=Dados!O{linha_dados}'
        ws_mix.cell(r, 5).number_format = '#,##0'
        ws_mix.cell(r, 5).alignment = right_align
        ws_mix.cell(r, 5).border = border
        
        ws_mix.cell(r, 6).value = f'=Dados!P{linha_dados}'
        ws_mix.cell(r, 6).number_format = '#,##0'
        ws_mix.cell(r, 6).alignment = right_align
        ws_mix.cell(r, 6).border = border
        
        ws_mix.cell(r, 7).value = f'=Dados!Q{linha_dados}'
        ws_mix.cell(r, 7).number_format = '#,##0'
        ws_mix.cell(r, 7).alignment = right_align
        ws_mix.cell(r, 7).border = border
    
    ws_mix.column_dimensions['A'].width = 10
    ws_mix.column_dimensions['B'].width = 35
    for col in range(3, 8):
        ws_mix.column_dimensions[get_column_letter(col)].width = 18
    ws_mix.freeze_panes = 'A4'
    
    # =========================================================================
    # ABA ANÁLISE MÉDIA ANUAL (com PROCV e SE)
    # =========================================================================
    ...
    # Critérios (em células para referência nas fórmulas)
    ws_analise['F1'] = 200000  # Limite faturamento
    ws_analise['F2'] = 0.10    # Limite margem (10% como decimal 0.10)
    ws_analise['F3'] = 20      # Limite mix
    
    for idx in range(len(df_resumo)):
        r = idx + 7
        linha_dados = idx + 2
        
        # RCA e Nome - referência direta
        cell_rca = ws_analise.cell(r, 1)
        cell_rca.value = f'=Dados!A{linha_dados}'
        cell_rca.border = border
        
        cell_nome = ws_analise.cell(r, 2)
        cell_nome.value = f'=Dados!B{linha_dados}'
        cell_nome.border = border
        
        # FAT - usar PROCV para buscar Faturamento Média Ano (coluna 4 da aba Dados)
        # PROCV busca pelo RCA (coluna A) na aba Dados
        # Intervalo completo até coluna Q para garantir que funcione
        cell_fat = ws_analise.cell(r, 3)
        cell_fat.value = f'=SE(PROCV($A{r};Dados!$A:$Q;4;FALSO)>=$F$1;"ACIMA";"ABAIXO")'
        cell_fat.border = border
        cell_fat.alignment = center_align
        
        # MARGEM - usar PROCV para buscar Margem Média Ano (coluna 9 da aba Dados)
        cell_margem = ws_analise.cell(r, 4)
        cell_margem.value = f'=SE(PROCV($A{r};Dados!$A:$Q;9;FALSO)>=$F$2;"ACIMA";"ABAIXO")'
        cell_margem.border = border
        cell_margem.alignment = center_align
        
        # MIX - usar PROCV para buscar Mix Média Ano (coluna 14 da aba Dados)
        cell_mix = ws_analise.cell(r, 5)
        cell_mix.value = f'=SE(PROCV($A{r};Dados!$A:$Q;14;FALSO)>=$F$3;"ACIMA";"ABAIXO")'
        cell_mix.border = border
        cell_mix.alignment = center_align
    
    # Resumo no final (usando CONT.SE)
    ...
    # Linha FATURAMENTO (usando CONT.SE)
    ...
    cell_fat_acima.value = f'=CONT.SE(C7:C{linha_inicio_resumo-1};"ACIMA")'
    ...
    cell_margem_acima.value = f'=CONT.SE(D7:D{linha_inicio_resumo-1};"ACIMA")'
    ...
    cell_mix_acima.value = f'=CONT.SE(E7:E{linha_inicio_resumo-1};"ACIMA")'
    ...
    cell_mix_abaixo.value = f'=CONT.SE(E7:E{linha_inicio_resumo-1};"ABAIXO")'
...
if __name__ == "__main__":
    ...
    root.mainloop()
# portfolio-commit-ready: analise_vendedores_excel_procv
